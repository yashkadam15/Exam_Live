...
import java.io.File;
/**
 * Author: Amogh,Yograj:For Client wise property loading
 */
...
public class ReloadableResourceBundleMessageSource extends AbstractMessageSource
	...
	protected List<String> calculateFilenamesForLocale(String basename, Locale locale) {
		...
		/*Start::Changes  for client wise property loading Yograj:15-July-2015*/
		Client client= new ExamVenueActivationServicesImpl().getClientDetails();
		if(client!=null && client.getClientID() > 0l)
		{
			if(basename.contains("clientmessage_"))
			{
				//Start: Check if client message file exist if not then load mkcl file : Yograj : 20-May-2019 
				if(!this.resourceLoader.getResource(basename+client.getClientID()+"_"+language+".properties").exists())
				{
					temp.append(1);//MKCL Client As default
				}
				else
				{
					temp.append(client.getClientID());
				}
				//End: Check if client message file exist if not then load mkcl file : Yograj : 20-May-2019 
				
				result.add(0, temp.toString());
			}
			else if(basename.equals("classpath:messages") && client.getPropertyFileBasename()!=null && !client.getPropertyFileBasename().trim().isEmpty())
			{
				temp=new StringBuilder("classpath:"+client.getPropertyFileBasename());
				result.add(0, temp.toString());
			}
		}
		else
		...
		/*End::Changes  for client wise property loading Yograj:15-July-2015*/
