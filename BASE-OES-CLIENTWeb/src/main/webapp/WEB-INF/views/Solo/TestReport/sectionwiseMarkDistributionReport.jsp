...
<html>
<head>
<title><spring:message code="MarkDistributionReport.title"/></title>
...
<body>
	<fieldset class="well">
		<div>
			<legend>
				<span><spring:message code="MarkDistributionReport.title"/>
		...
		<div class="holder">
			<form:form  class="form-horizontal"  action="sectionwisemarks" method="GET" onsubmit="return validate();">
	...
	<fieldset class="well">
			...
			<table class="table table-striped table-bordered" id="tblCandidateList" >
				<thead>
					...
					<c:choose>
						<c:when test="${ fn:length(viewModelList[0].sectionNameList)>1}">
							...
							<th rowspan="2" style="vertical-align: middle;text-align: center;"><spring:message code="MarkDistributionReport.TotalMarks"/></th>
							<tr>
						...
						<c:otherwise>
							...
							<th style="vertical-align: middle;text-align: center;"><spring:message code="MarkDistributionReport.TotalMarks"/></th>
						</c:otherwise>
					</c:choose>
				...
				<%-- <tfoot>
					<tr>
						...
						<th><spring:message code="MarkDistributionReport.CandidateCode"/></th>
						<c:forEach var="sectionName" items="${viewModelList[0].sectionNameList}" >
							<th>${sectionName}</th>
						</c:forEach>
						<th><spring:message code="MarkDistributionReport.TotalMarks"/></th>
					</tr>
				</tfoot> --%>
					<tbody>
						<c:forEach var="viewModel" items="${viewModelList}">
							<tr>
								...
								<c:forEach var="marks" items="${viewModel.sectionMarksList}">
									<td>${marks}<%-- <c:set var="totMarks" value="${totMarks+marks}" scope="page" /> --%></td>
								</c:forEach>
								<td>
									<c:choose>
										<c:when test="${viewModel.examCompleted==true}">
										 	<fmt:formatNumber type="number" maxFractionDigits="2" minFractionDigits="2" value="${viewModel.sectionTotalMarks}" />
										</c:when>
										<c:otherwise>
											<spring:message code="MarkDistributionReport.INCOMPLETE"/>
										</c:otherwise>
									</c:choose>								
								</td>
							</tr>
			...
			<div class="dataTables_paginate paging_bootstrap pagination" align="right">
					...
					<form:form action="sectionwisemarksprev" method="POST" modelAttribute="pagination"
					...
					<form:form action="sectionwisemarksnext" method="POST" modelAttribute="pagination"
	...
	$('#go').click(
			function() {
				...
				window.location.href = 'sectionwisemarks?searchText='
