...
<script>
	// Add event listener for custom event dispatched from EW.jsp
    window.document.addEventListener('redirectEvent', function(event) {
        var pi = event.detail.pi;
        var ei = event.detail.ei;

        console.log("Redirecting to showTestResult with pi:", pi, "and ei:", ei);
        // Perform redirection to showTestResult with the provided parameters
        window.location.href = "../endexam/showTestResult?candidateExamID=${candidateExam.candidateExamID}&examEventID="+ei+"&paperID="+pi+"&attemptNo=${candidateExam.attemptNo}&isTimeUp=true";
    });
</script>
...
console.log("PaperId TT: ",pi);
console.log("Exam event ID TT: ",ei);
</script>
			...
			<div class="span3">
				<div class="holder">
					<div class="profile-timer">
						...
						<!-- Place this code below the Exam.TimeRemaining section -->
<!-- Place this code below the Exam.TimeRemaining section -->
<p>Current Question Time Taken:</p>
<div id="stopwatch">00:00:00</div>
<input type="hidden" id="previousTimeInput" name="previousTime" >
<script>
  ...
  function stopStopwatch(fkItemID) {
	  elapsedTime = Math.floor((Date.now() - startTime) / 1000); // Convert milliseconds to seconds
	   ...
	   // sessionStorage.setItem('previousTime', previousTime);
	    ...
	    sessionStorage.setItem('ceid', ceid);
        var userId = document.getElementById('ci').value;
        ...
        document.getElementById('previousTimeInput').setAttribute('value', previousTime);
  ...
  function resetStopwatch(fkItemID) {
	stopStopwatch(fkItemID);
    elapsedTime = 0;
  ...
  startStopwatch(); // Start the stopwatch when the exam begins
</script>
						<!-- Start by harshadd -->
						...
						<input type="hidden" id="hidelapsedTimeDuration"
							value="${candidateExam.elapsedTime}" /> <input type="hidden"
							id="timeLeft" value="${timeLeft}" /> <input type="hidden"
							id="updateElapsedTime" value="${updateElapsedTime}" /> <span></span>
					</div>
						...
						<div class="legend">
							<span class="attempted"><span id="Cntans"
								class="btn btn-greener"></span> <spring:message
									code="Exam.Answered" /></span> <span class="current"><span
								id="CntNans" class="btn btn-red"></span> <spring:message
									code="Exam.NotAnswered" /></span> <span class="unattempted"><span
									...
									<c:choose>
										<c:when test="${candidateItemAssociation.object == 0}">
											<a data-qid="${candidateItemAssociation.fkItemID}"
												...
												onclick="resetStopwatch(${candidateItemAssociation.fkItemID})"
