								...
								<c:choose>
									<c:when test="${attemptStatusHeader!=1}">
										...
										<!--Display button for Personality profiling test(event id=54 and total papers=7 and single display category) -->
											<!-- //added or condition for MFS IGNOU Requirement   13-Nov-2019 -->
											<c:if test="${currentExamEvent.examEventID==54 || currentExamEvent.examEventID==1025}">
												<c:choose>
													<c:when test="${countOfCompletedPapers==7}">
														<a class="btn btn-blue pull-center" href="../profilingTestReport/candidateProfilingTestReport?eventid=${currentExamEvent.examEventID}&candidateUserName=${candidate.candidateCode}" target="_blank"><spring:message code="viewtestscoreForKlick.PrintCertificate" /></a>
													</c:when>
													<c:otherwise>
									...
									<tr>
										<td><c:choose>
												...
												<c:when test="${paper.examEventPaperDetails != null and paper.examEventPaperDetails.noOfAttempts == 2}">
													<span class="label label-attempt">Attempt #${paper.candidateExam.attemptNo}</span>
												</c:when>
											</c:choose></td>
										...
										<td class="text-center"><a class="btn btn-red viewSyllabusBtn"  data-value="${paper.paper.paperID}" ><spring:message code="homepage.activeViewsyllabus" /></a>  <c:choose>
												<c:when test="${paper.examEventPaperDetails.showAnalysis==true}">
													<a class="btn btn-warning" href="../ResultAnalysis/viewtestscore?examEventId=${paper.examEvent.examEventID}&paperId=${paper.paper.paperID}&attemptNo=${paper.candidateExam.attemptNo}">&nbsp;&nbsp;&nbsp;<spring:message code="homepage.historyViewAnalysis" />&nbsp;&nbsp;&nbsp;
													</a>
												</c:when>
												<c:otherwise>
													<a class="btn btn-disabled" href="javascript:void(0);">&nbsp;&nbsp;&nbsp;<spring:message code="homepage.historyViewAnalysis" />&nbsp;&nbsp;&nbsp;
													</a>
												</c:otherwise>
											</c:choose></td>
									</tr>
