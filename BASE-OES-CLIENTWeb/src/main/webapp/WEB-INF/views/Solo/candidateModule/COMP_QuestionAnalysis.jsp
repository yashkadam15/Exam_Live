...
<c:if test="${examViewModel.comprehension!=null}">
	<br>
	<b> <spring:message code="questionByquestion.mainQuestion" />
		${itemNo}:&nbsp;
	</b>
	<p class="question-wrap">${examViewModel.comprehension.itemText }</p>
	 <c:if test="${fn:length(examViewModel.comprehension.itemFilePath)!=0}">
		<img src="../exam/displayImage?disImg=${examViewModel.comprehension.itemFilePath}" class="resize" /><br>							
	</c:if> 
	<hr class="lineColor" />
	<c:set var="lineCnt" value="0" />
	<c:forEach var="subitemObj"
		items="${examViewModel.comprehension.subItemList}" varStatus="i1">
		...
		<c:if
			test="${fn:length(examViewModel.comprehension.subItemList)!=i1.count}">
			<hr class="lineColor" />
		</c:if>
