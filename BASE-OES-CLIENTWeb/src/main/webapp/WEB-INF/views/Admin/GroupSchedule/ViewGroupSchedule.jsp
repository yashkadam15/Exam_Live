...
<html>
<head>
<title><spring:message code="viewLabSession.Title"></spring:message></title>
...
<body>
	...
	<fieldset class="well">
		<div>
			<legend>
				<span><spring:message code="viewLabSession.Title"></spring:message></span>
			</legend>
		</div>
		
		<div class="holder">
			<form:form  class="form-horizontal"  action="viewLabSessionSchedule" method="post" onsubmit="return validate(this);">
				...
				<div class="control-group">
					<div class="controls">
						<button type="submit" class="btn btn-blue" ><spring:message code="viewLabSession.Title"></spring:message></button>
					</div>
				</div>
			</form:form>
		...
		<c:if test="${!flagCreateMode==true && fn:length(definedSheduleMap)==0}">
			...
			<div id="allWeekStatusGrid">
				...
				<table class="table table-striped table-bordered">
					<tbody>
							<tr>
								<th><spring:message code="scheduleExam.day"></spring:message></th>
						...
						<c:forEach items="${definedSheduleMap}" var="weekObj">
									...
									<c:choose>
										...
										<td rowspan="${fn:length(weekObj.value)+1}">
											...
											<fmt:formatDate value="${weekObj.key.scheduleStart}"
												...
												value="${weekObj.key.scheduleStart}" pattern="dd/MM/yyyy"
												var="startDateValue" /> <fmt:formatDate
												value="${weekObj.key.scheduleEnd}" pattern="dd/MM/yyyy"
												var="endDateValue" /> <c:if
												test="${startDateValue != endDateValue}">
												-<fmt:formatDate value="${weekObj.key.scheduleEnd}" type="date" />
											</c:if> 
										</td>
								...
								<c:otherwise>
									<tr>
										<td colspan="3">
											<fmt:formatDate value="${weekObj.key.scheduleStart}" type="date" />
												<fmt:formatDate value="${weekObj.key.scheduleStart}"
	 ...
	 $(document).ready(function(){
		...
		function displayDivision(response, selectedId) {
			...
			collectionType=response[0].collectionType;
			 ...
			 $("#collectionSelect").append("<option value='-1'><spring:message code="scheduleExam.select"/></option>"); 
