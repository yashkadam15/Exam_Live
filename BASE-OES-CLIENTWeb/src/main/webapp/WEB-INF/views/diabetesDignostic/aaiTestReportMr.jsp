	...
	<div style="margin-bottom:10px;">
		...
		<div class="content-wrapper">
				...
				<c:if test="${candidateExam.candidateExamID != null}">
					<div class="row header-info">
           				<div class="col-8 header-label">
							<strong><spring:message code="diabetesTestMr.Name"/></strong>
						...
						<div class="col-2 header-label">
			              <strong><spring:message code="diabetesTestMr.sex"/></strong>
			              <c:if test="${candidateExam.candidate.candidateGender=='M'}"><spring:message code="diabetesDiagnosticTestEnResult.male"/> <!-- पुरुष --> </c:if>
			              <c:if test="${candidateExam.candidate.candidateGender=='F'}"><spring:message code="diabetesDiagnosticTestEnResult.female"/><!-- स्त्री --> </c:if>
			            </div>  
			            <div class="col-2 header-label">
			              <strong><spring:message code="diabetesTestMr.age"/></strong>
			              <c:choose>
			              	<c:when test="${age==0}">
			              		<spring:message code="diabetesTest.NA"/>
			              	</c:when>
			              	<c:otherwise>
			              		${age} <spring:message code="diabetesTestMr.years"/>
			              	</c:otherwise>
			              </c:choose>
			            </div>  
					</div>
					<div class="row header-info">
			            <div class="col-4 header-label">
			              <strong><spring:message code="diabetesTestMr.centerCode"/></strong>
			               ${candidateExam.examVenue.examVenueCode}
			            </div>  
			            <div class="col-4 header-label">
			              <strong><spring:message code="diabetesTestMr.centerName"/></strong>
			               ${candidateExam.examVenue.examVenueName}
			            </div>  
			            <div class="col-4 header-label">
			              <strong><spring:message code="diabetesTestMr.district"/></strong>
			               ${candidateExam.examVenue.examVenuePinCode}
			            </div>  
			          </div>
			          <div class="row header-info">
			          	<div class="col header-label">
			              <strong><spring:message code="diabetesTestMr.address"/></strong>
			...
			<div class="content-area">
				<div class="content-text"><spring:message code="diabetesTestMr.dear"/>
           			<span class="content-username"> ${candidateExam.candidate.candidateFirstName} </span>,
          		</div>
          		 <div class="content-text">
            		<spring:message code="diabetesTestMr.mainContent"/>
            		<strong><spring:message code="diabetesTestMr.subContent"/></strong>
				</div>
		        	...
		        	<c:forEach items="${categoryList}" var="category" >
			        	<c:forEach items="${candidateSuggestionViewModelList}" var="sm" varStatus="i" >
			        		<c:if test="${sm.suggestionMaster.fkCategoryID == category.categoryID}">
			        		 <b> ${i.index+1} : ${category.categoryName}</b>
			        			<div class="content-text content-text-answers">
			        				${sm.suggestionMaster.suggestionText}
			        			</div>
			        		</c:if>
			        	</c:forEach>
		        	</c:forEach>
		        ...
		        <div class="content-text content-text-note">
	            	<span><spring:message code="diabetesTestMr.dueDate"/></span>
						${dueDate}
	          	</div>
